...
cfwarpIP() {
    if [[ ! -f "$PREFIX/bin/warpendpoint" ]]; then
        ...
        if [[ -n $cpu ]]; then
            curl -L -o warpendpoint -# --retry 2 https://github.com/bin1zone/warp-script-on-ish/blob/main/endip/$cpu
...
v2ray() {
  ...
  WireguardURL="wireguard://$(urlencode "$PrivateKey")@$Endip_v46?address=$(urlencode "$Address")&publickey=$(urlencode "$PublicKey")&mtu=$(urlencode "$MTU")#BINZONE_WireGuard"
